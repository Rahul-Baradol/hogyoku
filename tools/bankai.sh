...
i686-elf-gcc -c $BASE_PATH/kernel/kernel/terminal.c -o $BASE_PATH/build/terminal.o -std=gnu99 -ffreestanding -O2 -Wall -Wextra

i686-elf-gcc -T $BASE_PATH/kernel/arch/i386/linker.ld -o $BASE_PATH/bin/myos.bin -ffreestanding -O2 -nostdlib $CRTI_OBJ $CRTBEGIN_OBJ $BASE_PATH/build/boot.o $BASE_PATH/build/kernel.o $BASE_PATH/build/terminal.o $CRTEND_OBJ $CRTN_OBJ -lgcc
